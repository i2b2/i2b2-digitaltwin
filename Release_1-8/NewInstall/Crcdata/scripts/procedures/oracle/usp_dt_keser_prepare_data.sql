...
CREATE OR REPLACE PROCEDURE usp_dt_keser_prepare_data
AS
...
BEGIN
    ...
    dbms_output.put_line('  insert into dt_keser_patient_partition, step time (ms): ' || to_char(time_ms) || ', rows: ' || to_char(row_count));
    ...
    time_ms := extract (day from (localtimestamp - step_start_time))*24*60*60 +
        ...
        extract (minute from (localtimestamp - step_start_time))*60+
        round(extract(second from (localtimestamp - step_start_time))*1000);
    dbms_output.put_line('  insert into dt_keser_patient_period_feature, step time (ms): ' || to_char(time_ms) || ', rows: ' || to_char(row_count));
    ...
    time_ms := extract (day from (localtimestamp - step_start_time))*24*60*60 +
        ...
        extract (minute from (localtimestamp - step_start_time))*60+
        round(extract(second from (localtimestamp - step_start_time))*1000);
    dbms_output.put_line('  insert into dt_keser_feature_count, step time (ms): ' || to_char(time_ms) || ', rows: ' || to_char(row_count));
    ...
    time_ms := extract (day from (localtimestamp - step_start_time))*24*60*60 +
        ...
        extract (minute from (localtimestamp - step_start_time))*60+
        round(extract(second from (localtimestamp - step_start_time))*1000);
    dbms_output.put_line('  delete from dt_keser_patient_period_feature [0-99], step time (ms): ' || to_char(time_ms) || ', rows: ' || to_char(row_count));
    ...
    time_ms := extract (day from (localtimestamp - step_start_time))*24*60*60 +
        ...
        extract (minute from (localtimestamp - step_start_time))*60+
        round(extract(second from (localtimestamp - step_start_time))*1000);
    dbms_output.put_line('  delete from dt_keser_patient_period_feature [100-199], step time (ms): ' || to_char(time_ms) || ', rows: ' || to_char(row_count));
    ...
    while partition_start<200 loop
        ...
        time_ms := extract (day from (localtimestamp - step_start_time))*24*60*60 +
            ...
            extract (minute from (localtimestamp - step_start_time))*60+
            round(extract(second from (localtimestamp - step_start_time))*1000);
        dbms_output.put_line('  insert into dt_keser_feature_cooccur_temp [' || to_char(partition_start) || '-' || to_char(partition_end) || '], step time (ms): ' || to_char(time_ms) || ', rows: ' || to_char(row_count));
    ...
    time_ms := extract (day from (localtimestamp - step_start_time))*24*60*60 +
        ...
        extract (minute from (localtimestamp - step_start_time))*60+
        round(extract(second from (localtimestamp - step_start_time))*1000);
    dbms_output.put_line('  insert into dt_keser_feature_cooccur, step time (ms): ' || to_char(time_ms) || ', rows: ' || to_char(row_count));
    ...
    */
    ...
    dbms_output.put_line('  usp_dt_keser_prepare_data, total time (ms): ' || to_char(time_ms));
