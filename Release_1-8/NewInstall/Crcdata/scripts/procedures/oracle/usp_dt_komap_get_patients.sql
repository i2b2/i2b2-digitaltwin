...
CREATE OR REPLACE PROCEDURE usp_dt_komap_get_patients
AS
...
BEGIN
    ...
    */
    ...
    dbms_output.put_line('  insert into dt_komap_patient_feature, step time (ms): ' || to_char(time_ms) || ', rows: ' || to_char(row_count));
    ...
    from (
        ...
        where f.concept_cd like 'DIAG|ICD%' -- or 'ICD%' for ACT ontology
    ...
    time_ms := extract (day from (localtimestamp - step_start_time))*24*60*60 +
        ...
        extract (minute from (localtimestamp - step_start_time))*60+
        round(extract(second from (localtimestamp - step_start_time))*1000);
    dbms_output.put_line('  insert into dt_komap_patient_feature (Utilization:IcdDates), step time (ms): ' || to_char(time_ms) || ', rows: ' || to_char(row_count));
    ...
    time_ms := extract (day from (localtimestamp - step_start_time))*24*60*60 +
        ...
        extract (minute from (localtimestamp - step_start_time))*60+
        round(extract(second from (localtimestamp - step_start_time))*1000);
    dbms_output.put_line('  insert into dt_komap_patient_feature (DEM|SEX:F), step time (ms): ' || to_char(time_ms) || ', rows: ' || to_char(row_count));
    ...
    time_ms := extract (day from (localtimestamp - step_start_time))*24*60*60 +
        ...
        extract (minute from (localtimestamp - step_start_time))*60+
        round(extract(second from (localtimestamp - step_start_time))*1000);
    dbms_output.put_line('  insert into dt_komap_patient_feature (DEM|AGE), step time (ms): ' || to_char(time_ms) || ', rows: ' || to_char(row_count));
    ...
    time_ms := extract (day from (localtimestamp - step_start_time))*24*60*60 +
        ...
        extract (minute from (localtimestamp - step_start_time))*60+
        round(extract(second from (localtimestamp - step_start_time))*1000);
    dbms_output.put_line('  insert into dt_komap_patient_feature (VITAL|BMI), step time (ms): ' || to_char(time_ms) || ', rows: ' || to_char(row_count));
    ...
    time_ms := extract (day from (localtimestamp - step_start_time))*24*60*60 +
        ...
        extract (minute from (localtimestamp - step_start_time))*60+
        round(extract(second from (localtimestamp - step_start_time))*1000);
    dbms_output.put_line('  insert into dt_komap_patient_feature (VITAL|SMOKING), step time (ms): ' || to_char(time_ms) || ', rows: ' || to_char(row_count));
    ...
    */
    ...
    time_ms := extract (day from (localtimestamp - step_start_time))*24*60*60 +
        ...
        extract (minute from (localtimestamp - step_start_time))*60+
        round(extract(second from (localtimestamp - step_start_time))*1000);
    dbms_output.put_line('  insert into dt_komap_base_cohort, step time (ms): ' || to_char(time_ms) || ', rows: ' || to_char(row_count));
    ...
    dbms_output.put_line('  usp_dt_komap_get_patients, total time (ms): ' || to_char(time_ms));
