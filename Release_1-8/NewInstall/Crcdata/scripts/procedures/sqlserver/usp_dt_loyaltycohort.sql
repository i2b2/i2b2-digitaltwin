...
IF OBJECT_ID(N'dbo.CONCEPT', N'U') IS NOT NULL
   OR OBJECT_ID(N'dbo.CONCEPT', N'V') IS NOT NULL
   OR OBJECT_ID(N'dbo.CONCEPT', N'SN') IS NOT NULL
BEGIN
  -- ACT-OMOP ontology base codes are concept IDs. Translate them back to
  -- vocabulary/code strings so the published ICD Charlson patterns still apply.
  ...
  JOIN dbo.CONCEPT OC
    ON CONVERT(varchar(50), OC.CONCEPT_ID) = DX.CONCEPT_CD
    AND CONCAT(OC.VOCABULARY_ID, ':', OC.CONCEPT_CODE) LIKE C.DIAGPATTERN
    AND OC.CONCEPT_CODE NOT LIKE '%-%'
END
ELSE
BEGIN
