...
IF OBJECT_ID(N'tempdb..#DT_LOYALTY_CONCEPT_DIMENSION', N'U') IS NOT NULL DROP TABLE #DT_LOYALTY_CONCEPT_DIMENSION;

-- BUILD A PRIVATE UNIFIED CONCEPT MAP.
-- Classic i2b2 has one CONCEPT_DIMENSION table. ACT-OMOP splits the ontology
-- across the tables registered in TABLE_ACCESS, so load those tables into the
-- same temporary shape without changing the shared ACT-OMOP implementation.
CREATE TABLE #DT_LOYALTY_CONCEPT_DIMENSION (
  CONCEPT_PATH varchar(700) NOT NULL,
  CONCEPT_CD varchar(50) NOT NULL
)

IF OBJECT_ID(N'dbo.CONCEPT_DIMENSION', N'U') IS NULL
   AND OBJECT_ID(N'dbo.CONCEPT_DIMENSION', N'V') IS NULL
   AND OBJECT_ID(N'dbo.CONCEPT_DIMENSION', N'SN') IS NULL
   AND OBJECT_ID(N'dbo.TABLE_ACCESS', N'U') IS NULL
   AND OBJECT_ID(N'dbo.TABLE_ACCESS', N'V') IS NULL
   AND OBJECT_ID(N'dbo.TABLE_ACCESS', N'SN') IS NULL
BEGIN
  RAISERROR(N'Loyalty cohort requires dbo.CONCEPT_DIMENSION or ACT-OMOP dbo.TABLE_ACCESS metadata.', 16, 1)
  RETURN
END

-- Load a conventional unified concept dimension when present. Some hybrid
-- installations expose this as a synonym but omit dimension concepts such as
-- ACT-OMOP visit types, so TABLE_ACCESS is also loaded below when available.
IF OBJECT_ID(N'dbo.CONCEPT_DIMENSION', N'U') IS NOT NULL
   OR OBJECT_ID(N'dbo.CONCEPT_DIMENSION', N'V') IS NOT NULL
   OR OBJECT_ID(N'dbo.CONCEPT_DIMENSION', N'SN') IS NOT NULL
BEGIN
  INSERT INTO #DT_LOYALTY_CONCEPT_DIMENSION (CONCEPT_PATH, CONCEPT_CD)
  SELECT DISTINCT CONVERT(varchar(700), CONCEPT_PATH), CONVERT(varchar(50), CONCEPT_CD)
  FROM dbo.CONCEPT_DIMENSION
  WHERE CONCEPT_PATH IS NOT NULL AND NULLIF(CONCEPT_CD, '') IS NOT NULL
END

-- Merge all registered ontology tables rather than treating TABLE_ACCESS and
-- CONCEPT_DIMENSION as mutually exclusive metadata sources.
IF OBJECT_ID(N'dbo.TABLE_ACCESS', N'U') IS NOT NULL
   OR OBJECT_ID(N'dbo.TABLE_ACCESS', N'V') IS NOT NULL
   OR OBJECT_ID(N'dbo.TABLE_ACCESS', N'SN') IS NOT NULL
BEGIN

  DECLARE @ONTOLOGY_TABLE varchar(400)
  DECLARE @ONTOLOGY_OBJECT nvarchar(1035)
  DECLARE @ONTOLOGY_SQL nvarchar(max)

  DECLARE ONTOLOGY_CUR CURSOR LOCAL FAST_FORWARD FOR
    SELECT DISTINCT C_TABLE_NAME
    FROM dbo.TABLE_ACCESS
    WHERE NULLIF(C_TABLE_NAME, '') IS NOT NULL
      AND C_VISUALATTRIBUTES LIKE '%A%'

  OPEN ONTOLOGY_CUR
  FETCH NEXT FROM ONTOLOGY_CUR INTO @ONTOLOGY_TABLE

  WHILE @@FETCH_STATUS = 0
  BEGIN
    -- Support table, schema.table, database.schema.table, and linked-server
    -- four-part names while quoting every identifier component.
    SET @ONTOLOGY_OBJECT =
      CASE
        WHEN PARSENAME(@ONTOLOGY_TABLE, 4) IS NOT NULL THEN
          QUOTENAME(PARSENAME(@ONTOLOGY_TABLE, 4)) + N'.' + QUOTENAME(PARSENAME(@ONTOLOGY_TABLE, 3)) + N'.' + QUOTENAME(PARSENAME(@ONTOLOGY_TABLE, 2)) + N'.' + QUOTENAME(PARSENAME(@ONTOLOGY_TABLE, 1))
        WHEN PARSENAME(@ONTOLOGY_TABLE, 3) IS NOT NULL THEN
          QUOTENAME(PARSENAME(@ONTOLOGY_TABLE, 3)) + N'.' + QUOTENAME(PARSENAME(@ONTOLOGY_TABLE, 2)) + N'.' + QUOTENAME(PARSENAME(@ONTOLOGY_TABLE, 1))
        WHEN PARSENAME(@ONTOLOGY_TABLE, 2) IS NOT NULL THEN
          QUOTENAME(PARSENAME(@ONTOLOGY_TABLE, 2)) + N'.' + QUOTENAME(PARSENAME(@ONTOLOGY_TABLE, 1))
        ELSE N'dbo.' + QUOTENAME(PARSENAME(@ONTOLOGY_TABLE, 1))
      END

    IF @ONTOLOGY_OBJECT IS NULL
    BEGIN
      CLOSE ONTOLOGY_CUR
      DEALLOCATE ONTOLOGY_CUR
      RAISERROR(N'TABLE_ACCESS contains an invalid C_TABLE_NAME.', 16, 1)
      RETURN
    END

    SET @ONTOLOGY_SQL = N'
      INSERT INTO #DT_LOYALTY_CONCEPT_DIMENSION (CONCEPT_PATH, CONCEPT_CD)
      SELECT DISTINCT CONVERT(varchar(700), C_FULLNAME), CONVERT(varchar(50), C_BASECODE)
      FROM ' + @ONTOLOGY_OBJECT + N'
      WHERE C_FULLNAME IS NOT NULL
        AND NULLIF(CONVERT(varchar(50), C_BASECODE), '''') IS NOT NULL
        AND (
          C_FULLNAME LIKE ''\ACT\Demographics%''
          OR C_FULLNAME LIKE ''\ACT\Diagnosis\ICD10%''
          OR C_FULLNAME LIKE ''\ACT\Diagnosis\ICD9%''
          OR EXISTS (
            SELECT 1 FROM dbo.DT_LOYALTY_PATHS L
            WHERE L.CONCEPT_PATH IS NOT NULL
              AND L.CONCEPT_PATH <> ''**Not Found''
              AND C_FULLNAME LIKE L.CONCEPT_PATH + ''%''
          )
        )'

    BEGIN TRY
      EXEC sys.sp_executesql @ONTOLOGY_SQL
    END TRY
    BEGIN CATCH
      DECLARE @ONTOLOGY_ERROR nvarchar(2048) =
        N'Unable to read ontology table registered in TABLE_ACCESS: ' + @ONTOLOGY_TABLE
        + N'. ' + ERROR_MESSAGE()
      CLOSE ONTOLOGY_CUR
      DEALLOCATE ONTOLOGY_CUR
      RAISERROR(N'%s', 16, 1, @ONTOLOGY_ERROR)
      RETURN
    END CATCH
...
CREATE CLUSTERED INDEX CI_DT_LOYALTY_CONCEPT_DIMENSION
  ON #DT_LOYALTY_CONCEPT_DIMENSION (CONCEPT_PATH, CONCEPT_CD)

IF NOT EXISTS (
  SELECT 1
  FROM #DT_LOYALTY_CONCEPT_DIMENSION C
  JOIN dbo.DT_LOYALTY_PATHS L ON C.CONCEPT_PATH LIKE L.CONCEPT_PATH + '%'
  WHERE L.CONCEPT_PATH IS NOT NULL AND L.CONCEPT_PATH <> '**Not Found'
)
BEGIN
  RAISERROR(N'No DT_LOYALTY_PATHS entries matched the installed ontology. Check TABLE_ACCESS and the installed ACT ontology version.', 16, 1)
  RETURN
END

DECLARE @UNMATCHED_LOYALTY_PATHS int = (
  SELECT COUNT(*)
  FROM dbo.DT_LOYALTY_PATHS L
  WHERE L.CONCEPT_PATH IS NOT NULL
    AND L.CONCEPT_PATH <> '**Not Found'
    AND NOT EXISTS (
      SELECT 1 FROM #DT_LOYALTY_CONCEPT_DIMENSION C
      WHERE C.CONCEPT_PATH LIKE L.CONCEPT_PATH + '%'
    )
)

IF @UNMATCHED_LOYALTY_PATHS > 0
  RAISERROR(N'Warning: %d DT_LOYALTY_PATHS rows did not match the installed ontology. Run the OMOP validation script for details.', 1, 1, @UNMATCHED_LOYALTY_PATHS) WITH NOWAIT
...
BEGIN
  ...
  INTO #DEMCONCEPT
  FROM #DT_LOYALTY_CONCEPT_DIMENSION
...
FROM #INCLPAT P
  ...
  JOIN DBO.VISIT_DIMENSION V 
    ...
    AND LTRIM(RTRIM(CONVERT(varchar(50), V.INOUT_CD))) IN (SELECT LTRIM(RTRIM(VD.CONCEPT_CD))
                        FROM #DT_LOYALTY_CONCEPT_DIMENSION VD
                          JOIN DBO.DT_LOYALTY_PATHS LCP
                            ON VD.CONCEPT_PATH LIKE CONCAT(LCP.CONCEPT_PATH,'%')
...
CROSS APPLY (VALUES (
  CASE UPPER(LTRIM(RTRIM(CONVERT(varchar(50), P.SEX_CD))))
    WHEN 'DEM|SEX:F' THEN 'F'
    WHEN 'F' THEN 'F'
    WHEN '8532' THEN 'F' -- OMOP Female
    WHEN 'DEM|SEX:M' THEN 'M'
    WHEN 'M' THEN 'M'
    WHEN '8507' THEN 'M' -- OMOP Male
    ELSE REPLACE(UPPER(LTRIM(RTRIM(CONVERT(varchar(50), P.SEX_CD)))), 'DEM|SEX:', '')
  END
)) S(SEX)
GROUP BY COHORT_NAME, C.PATIENT_NUM, S.SEX, FLOOR(DATEDIFF(DD,P.BIRTH_DATE,INDEX_DT)/365.25), INDEX_DT

DECLARE @INP1_OPT1_VISIT_PATIENTS int
DECLARE @OPT2_VISIT_PATIENTS int
DECLARE @ED_VISIT_PATIENTS int

SELECT
  @INP1_OPT1_VISIT_PATIENTS = SUM(CONVERT(int, INP1_OPT1_VISIT)),
  @OPT2_VISIT_PATIENTS = SUM(CONVERT(int, OPT2_VISIT)),
  @ED_VISIT_PATIENTS = SUM(CONVERT(int, ED_VISIT))
FROM #COHORT

RAISERROR(
  N'Visit flag checkpoint - INP1_OPT1 patients=%d, OPT2 patients=%d, ED patients=%d',
  1, 1,
  @INP1_OPT1_VISIT_PATIENTS, @OPT2_VISIT_PATIENTS, @ED_VISIT_PATIENTS
) WITH NOWAIT
...
FROM DBO.DT_LOYALTY_PATHS L, #DT_LOYALTY_CONCEPT_DIMENSION C
...
CREATE TABLE #CHARLSON_DX (
  CHARLSON_CATGRY varchar(50),
  CHARLSON_WT int,
  CONCEPT_CD varchar(50)
)

-- ACT-OMOP stores the ICD code used by the Charlson definitions in
-- I2B2_ACT_BASECODE, but stores the fact-facing CONDITION_VIEW and
-- CONDITION_NS_VIEW concept IDs in OMOP_S_CONCEPT_ID and
-- OMOP_NS_CONCEPT_ID.  C_BASECODE alone cannot connect those two sides.
IF (OBJECT_ID(N'dbo.TABLE_ACCESS', N'U') IS NOT NULL
    OR OBJECT_ID(N'dbo.TABLE_ACCESS', N'V') IS NOT NULL
    OR OBJECT_ID(N'dbo.TABLE_ACCESS', N'SN') IS NOT NULL)
BEGIN
  DECLARE @CHARLSON_ONTOLOGY_TABLE_COUNT int = 0

  DECLARE CHARLSON_ONTOLOGY_CUR CURSOR LOCAL FAST_FORWARD FOR
    SELECT DISTINCT C_TABLE_NAME
    FROM dbo.TABLE_ACCESS
    WHERE NULLIF(C_TABLE_NAME, '') IS NOT NULL
      AND C_VISUALATTRIBUTES LIKE '%A%'
      AND (C_FULLNAME LIKE '\ACT\Diagnosis\ICD10%'
           OR C_FULLNAME LIKE '\ACT\Diagnosis\ICD9%')

  OPEN CHARLSON_ONTOLOGY_CUR
  FETCH NEXT FROM CHARLSON_ONTOLOGY_CUR INTO @ONTOLOGY_TABLE

  WHILE @@FETCH_STATUS = 0
  BEGIN
    SET @CHARLSON_ONTOLOGY_TABLE_COUNT += 1
    ...
    SET @ONTOLOGY_SQL = N'
      INSERT INTO #CHARLSON_DX (CHARLSON_CATGRY, CHARLSON_WT, CONCEPT_CD)
      SELECT DISTINCT C.CHARLSON_CATGRY, C.CHARLSON_WT, X.CONCEPT_CD
      FROM ' + @ONTOLOGY_OBJECT + N' DX
      JOIN dbo.DT_LOYALTY_CHARLSON C
        ON CONVERT(varchar(100), DX.I2B2_ACT_BASECODE) LIKE C.DIAGPATTERN
      CROSS APPLY (VALUES
        (CONVERT(varchar(50), DX.OMOP_S_CONCEPT_ID)),
        (CONVERT(varchar(50), DX.OMOP_NS_CONCEPT_ID))
      ) X(CONCEPT_CD)
      WHERE NULLIF(X.CONCEPT_CD, '''') IS NOT NULL
        AND CONVERT(varchar(100), DX.I2B2_ACT_BASECODE) NOT LIKE ''%-%'''

    BEGIN TRY
      EXEC sys.sp_executesql @ONTOLOGY_SQL
    END TRY
    BEGIN CATCH
      DECLARE @CHARLSON_ONTOLOGY_ERROR nvarchar(2048) =
        N'Unable to read ACT-OMOP Charlson columns from ontology table: '
        + @ONTOLOGY_TABLE + N'. ' + ERROR_MESSAGE()
      CLOSE CHARLSON_ONTOLOGY_CUR
      DEALLOCATE CHARLSON_ONTOLOGY_CUR
      RAISERROR(N'%s', 16, 1, @CHARLSON_ONTOLOGY_ERROR)
      RETURN
    END CATCH
  ...
  RAISERROR(N'Charlson metadata discovery - Diagnosis ontology tables: %d', 1, 1,
    @CHARLSON_ONTOLOGY_TABLE_COUNT) WITH NOWAIT
END
ELSE
BEGIN
  -- Classic i2b2 stores vocabulary-prefixed diagnosis codes directly.
  INSERT INTO #CHARLSON_DX (CHARLSON_CATGRY, CHARLSON_WT, CONCEPT_CD)
  SELECT DISTINCT C.CHARLSON_CATGRY, C.CHARLSON_WT, DX.CONCEPT_CD
  FROM dbo.DT_LOYALTY_CHARLSON C
  JOIN #DT_LOYALTY_CONCEPT_DIMENSION DX
    ON (DX.CONCEPT_PATH LIKE '\ACT\Diagnosis\ICD10%'
        OR DX.CONCEPT_PATH LIKE '\ACT\Diagnosis\ICD9%')
    AND DX.CONCEPT_CD LIKE C.DIAGPATTERN
    AND DX.CONCEPT_CD NOT LIKE '%-%'
END

DECLARE @CHARLSON_CODE_COUNT int = (SELECT COUNT(DISTINCT CONCEPT_CD) FROM #CHARLSON_DX)
DECLARE @CHARLSON_CATEGORY_COUNT int = (SELECT COUNT(DISTINCT CHARLSON_CATGRY) FROM #CHARLSON_DX)
DECLARE @CHARLSON_PATTERN_COUNT int = (SELECT COUNT(*) FROM dbo.DT_LOYALTY_CHARLSON)
RAISERROR(N'Charlson diagnosis map - Reference patterns: %d - Fact concept IDs: %d - Categories: %d', 1, 1,
  @CHARLSON_PATTERN_COUNT, @CHARLSON_CODE_COUNT, @CHARLSON_CATEGORY_COUNT) WITH NOWAIT

IF @CHARLSON_CODE_COUNT = 0
  RAISERROR(N'Warning: no Charlson diagnosis concepts matched the installed ontology; feature flags will be zero and CHARLSON_INDEX will contain age points only.', 1, 1) WITH NOWAIT
...
DECLARE @CHARLSON_FLAG_PATIENT_COUNT int = (
  SELECT COUNT(*)
  FROM #COHORT_CHARLSON
  WHERE MI + CHF + CVD + PVD + DEMENTIA + COPD + RHEUMDIS + PEPULCER
        + MILDLIVDIS + DIABETES_NOCC + DIABETES_WTCC + HEMIPARAPLEG
        + RENALDIS + CANCER + MSVLIVDIS + METASTATIC + AIDSHIV > 0
)
RAISERROR(N'Charlson diagnosis matches - Patients with one or more feature flags: %d', 1, 1,
  @CHARLSON_FLAG_PATIENT_COUNT) WITH NOWAIT
